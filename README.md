...
<p align="center">
  <strong>AI-to-Todoist automation bridge built with FastAPI, Pydantic, and Google Tasks API.</strong>
</p>
...
A robust Python bridge that parses structured task JSON generated by AI LLMs (Gemini, ChatGPT, Claude) or automation workflows (Google Tasks, n8n, Make) and batch-creates them in Todoist via the official REST API with automatic project mapping, due date assignment, and validation.
...
- **Google Tasks 2-Way Sync (`sync_worker.py`):** OAuth 2.0 daemon that reads tasks from Google Tasks, extracts tags (`#Project`, `p[1-4]`, `@Date`), pushes them to Todoist, and removes them from Google Tasks.
...
- **Google Tasks İki Yönlü Senkronizasyon (`sync_worker.py`):** Google Tasks'teki görevleri okur, etiketleri (`#Proje`, `p[1-4]`, `@Tarih`) ayrıştırır, Todoist'e ekler ve aktarılanları Google Tasks'ten siler.
...
- **FastAPI Webhook Sunucusu (`app.py`):** CORS ve `X-Bridge-Token` güvenlikli REST API servisi.
