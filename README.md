...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-98%20Passed-brightgreen?logo=pytest&logoColor=white" alt="Pytest" />
...
flowchart TD
    ...
    subgraph Gateways["🚀 Gateways & Interfaces"]
        C1["🤖 FastMCP Server (todoist_mcp.py)\n[Task, Label & Project Tools, Subprojects, Reopen]"]
...
- **🤖 Model Context Protocol (MCP) Server (`todoist_mcp.py`):** FastMCP server over STDIO for Claude Desktop with comprehensive task management (`create_task`, `list_tasks`, `complete_task`, `reopen_task`, `update_task`, `delete_task`), label support (`list_labels`, `create_label`), and hierarchical nested projects (`create_project`, `delete_project`, `list_projects`) with sanitized error handling.
...
├── tests/                  # Pytest test suite (98 unit & integration tests)
...
- **🤖 Model Context Protocol (MCP) Sunucusu (`todoist_mcp.py`):** Claude Desktop için FastMCP mimarisiyle yerel STDIO üzerinden çalışan; görev (`create_task`, `list_tasks`, `complete_task`, `reopen_task`, `update_task`, `delete_task`), etiket (`list_labels`, `create_label`) ve hiyerarşik alt proje (`create_project`, `delete_project`, `list_projects`) yönetim araçları.
...
├── tests/                  # Pytest test paketi (98 birim ve entegrasyon testi)
...
Projede 98 adet birim ve entegrasyon testi yer almaktadır:
